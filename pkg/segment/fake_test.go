...
func getEvent(userProps map[string]interface{}, eventProps map[string]interface{}) track.Traceable {
	return &testEvent{
...
func Test_Example_WithOpts(t *testing.T) {
	...
	err := f.Track(ctx, eventName, event, track.WithUserID("user_id"), track.WithAnonymousID("anonymous_id"))
