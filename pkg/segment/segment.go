...
func (st *segmentTracker) Track(ctx context.Context, eventName string, event track.Traceable, opts ...track.TrackOption) error {
	...
	if event.UserProperties() != nil {
		...
		if trackOpts.GetUserID() != "" {
			m.UserId = trackOpts.GetUserID()
		}
		if trackOpts.GetAnonymousID() != "" {
			m.AnonymousId = trackOpts.GetAnonymousID()
		}
		if trackOpts.GetEmail() != "" {
			m.UserId = trackOpts.GetEmail()
		}
